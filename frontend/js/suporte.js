...
async function carregarChamados(status, prioridade) {
    ...
    for (let i = 0; i < dados.chamados.length; i++) {
        const chamado = dados.chamados[i];
...
let nivelSuporte = false;
async function chamados() {
    const permissao = await carregarChamados(selectStatus.value, selectPrioriade.value)
    
    nivelSuporte = permissao;
}
