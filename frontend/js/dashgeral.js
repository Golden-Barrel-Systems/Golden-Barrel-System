    ...
    fetch(`/sensor/temperaturaAtual/${sensores[i].id_sensor}`, {
      ...
      .then((res) => {
        res.json().then((json) => {
          ...
          if (
            temperaturaAtual >= temperaturaIdeal - 1 &&
            temperaturaAtual <= temperaturaIdeal + 1
          ) {
...
function exibirAlertas(alertas) {
  ...
  for (let i = 0; i < alertas.length; i++) {
    if (
      alertas[i].mensagem == "Umidade acima do ideal" ||
      alertas[i].mensagem == "Umidade abaixo do ideal"
    ) {
      listaAlertas.innerHTML += `
...
async function carregarDados() {
  try {
    ...
    const respostaSensores = await fetch(
      `/camara/listarSensores/${camaraSelecionada}`,
      {
        method: "GET",
      },
    );
    ...
    for (let i = 0; i < sensores.length; i++) {
      ...
      const respostaMedicoes = await fetch(
        `/sensor/ultimasMedicoes/${sensor.id_sensor}`,
        {
          method: "GET",
        },
      );
      ...
      console.log(
        `Medições do sensor ${i} (ID: ${sensor.id_sensor}):`,
        medicoes,
      );
      ...
      console.log(`Dados processados sensor ${i}:`, {
        labelsTempLocal,
        dadosTempLocal,
        labelsUmiLocal,
        dadosUmiLocal,
